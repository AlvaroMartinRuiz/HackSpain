...
from v2.models import CallState, Operation, SchedulingCriteria, TurnDecision
...
class IncrementalWorkflowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_structured_output_defaults_do_not_erase_collected_insurance(self):
        await self.offer()
        await self.turn("I have Sanitas", {"op": "prepare", "intent_id": "one", "evidence": "Sanitas",
                                           "criteria": {"insurers": ["sanitas"]}})
        self.assertEqual(self.state.intents["one"].criteria.insurers, ["sanitas"])
        full_schema = SchedulingCriteria(when="Friday").model_dump()
        await self.turn("Friday instead", {"op": "prepare", "intent_id": "one", "evidence": "Friday",
                                           "criteria": full_schema})
        self.assertEqual(self.state.intents["one"].criteria.insurers, ["sanitas"])
        await self.turn("Remove that plan", {"op": "prepare", "intent_id": "one", "evidence": "Remove that plan",
                                             "clear_fields": ["insurers"], "criteria": full_schema})
        self.assertEqual(self.state.intents["one"].criteria.insurers, [])
