...
from v2.domain.catalog import Catalog, WEEKDAY_NAMES
from v2.domain.engine import SchedulingEngine
from v2.domain.identity import normalize_phone, normalize_provider_name, parse_national_id
from v2.domain.timeref import matches_part_of_day, parse_slot
...
class ClinicBoundary:
    ...
    async def identify_result(self, identity: Identity, today: date | None = None) -> dict:
        ...
        for key in ("date_of_birth", "phone", "national_id"):
            if not query.get(key) or not patient.get(key):
                continue
            actual = str(patient[key])
            if key == "phone":
                actual = normalize_phone(actual)
            elif key == "national_id":
                parsed = parse_national_id(actual)
                actual = str(parsed["value"]) if parsed["valid"] else None
            if actual != query[key]:
