...
- Paid execution requires explicit opt-in (`V2_ENABLE_PAID=true`). v2 no longer tracks a local spending ledger or cap — it never settled real cost and only accumulated stale reservations that blocked legitimate calls, so it was removed. Stay mindful of the shared $30 effort budget by watching each provider's own dashboard directly (Vercel AI Gateway, Cartesia, ElevenLabs, Deepgram); nothing in-app enforces it anymore.
